        ...
        extends AbstractTestFunctions
{
    ...
    private void testAggregationDoubles(InternalAggregationFunction function, Page page, double compression, double... inputs)
    {
        ...
        assertValueWithinError(StandardTypes.DOUBLE, returned, compression, rows, 0.1, 0.5, 0.9, 0.99);
    }
    ...
    private void assertValueWithinError(String type, SqlVarbinary binary, double error, List<? extends Number> rows, double percentile)
    {
        ...
        functionAssertions.assertFunction(
                format("quantile_at_value(CAST(X'%s' AS tdigest(%s)), %s) >= %s", binary.toString().replaceAll("\\s+", " "),
                        type,
    ...
    private Number getLowerBoundQuantile(double quantile, double bound)
    {
        return Math.max(0, quantile - bound);
    }

    private Number getUpperBoundQuantile(double quantile, double bound)
    {
