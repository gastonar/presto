...
public final class TDigestFunctions
{
    ...
    {
        return TDigest.createMergingDigest(input).getQuantile(quantile);
    }
    ...
    {
        TDigest digest = TDigest.createMergingDigest(input);
        BlockBuilder output = DOUBLE.createBlockBuilder(null, percentilesArrayBlock.getPositionCount());
        for (int i = 0; i < percentilesArrayBlock.getPositionCount(); i++) {
            DOUBLE.writeDouble(output, digest.getQuantile(DOUBLE.getDouble(percentilesArrayBlock, i)));
        }
        return output.build();
    }
