...
public class TDigestUtils
{
    ...
    {
        checkArgument(x1 <= x2, "error");
        final double x = (x1 * w1 + x2 * w2) / (w1 + w2);
        return Math.max(x1, Math.min(x, x2));
    }

    // Scale Functions
    public static double qToK(double q, double compression, double n)
    {
        if (n <= 1) {
            if (q <= 0) {
                return -10;
        ...
        if (q == 0) {
            return 2 * qToK(1 / n, compression, n);
        }
        else if (q == 1) {
            return 2 * qToK((n - 1) / n, compression, n);
        }
        else {
    ...
    public static double qToK(double q, double normalizer)
    {
        if (q < 1e-15) {
            // this will return something more extreme than q = 1/n
            return 2 * qToK(1e-15, normalizer);
        }
        else if (q > 1 - 1e-15) {
            // this will return something more extreme than q = (n-1)/n
            return 2 * qToK(1 - 1e-15, normalizer);
        }
        else {
    ...
    public static double kToQ(double k, double compression, double n)
    {
        double w = Math.exp(k * Z(compression, n) / compression);
        return w / (1 + w);
    }

    public static double kToQ(double k, double normalizer)
    {
        double w = Math.exp(k / normalizer);
        return w / (1 + w);
    }

    public static double max(double q, double compression, double n)
    {
        return Z(compression, n) * q * (1 - q) / compression;
    }

    public static double max(double q, double normalizer)
    {
