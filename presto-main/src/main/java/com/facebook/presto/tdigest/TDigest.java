...
import static com.facebook.presto.tdigest.TDigestUtils.max;
...
public class TDigest
{
    ...
    public double getQuantile(double q)
    {
        ...
        checkArgument(weight[n - 1] > 1, String.format("Expected weight[n - 1] > 1, but was %s", weight[n - 1]));
        checkArgument(index <= totalWeight, String.format("Expected index <= totalWeight, but index:%s > totalWeight:%s", index, totalWeight));
        checkArgument(index >= totalWeight - weight[n - 1] / 2, String.format("Expected index >= totalWeight - weight[n - 1] / 2, but" +
