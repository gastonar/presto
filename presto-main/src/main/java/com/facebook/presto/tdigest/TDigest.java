...
import static com.facebook.presto.tdigest.TDigestUtils.kToQ;
...
import static com.facebook.presto.tdigest.TDigestUtils.qToK;
...
public class TDigest
{
    ...
    public TDigest(double compression)
    {
        ...
        publicCompression = max(compression, 10);
        ...
        int bufferSize = 5 * (int) ceil(publicCompression + sizeFudge);
    ...
    {
        checkArgument(this.publicCompression == other.getCompressionFactor(), format("Cannot merge two t-digest with different compression factors (%s, %s)",
                this.publicCompression, other.getCompressionFactor()));
            ...
            double unmergedWeight, boolean runBackwards, double compression)
    {
        ...
        checkArgument((activeCentroids + incomingCount) > 0, format("Active centroids plus incoming count must be > 0, was %s", activeCentroids + incomingCount));
        activeCentroids = 0;
        ...
        checkArgument(sum == totalWeight, format("Sum must equal the total weight, but sum:%s != totalWeight:%s", sum, totalWeight));
        if (runBackwards) {
    ...
    {
        checkArgument(x >= min && x <= max, format("X must be in range of t-digest, between[%s, %s]", min, max));
        if (unmergedWeight > 0) {
            compress();
        }
        ...
        if (activeCentroids == 0) {
            ...
            checkArgument(x >= mean[0], format("Value x:%s must be greater than mean of first centroid %s if we got here", x, mean[0]));
            ...
            for (int it = 0; it < n - 1; it++) {
                ...
                if (mean[it] == x) {
                    ...
                    if (mean[it + 1] - mean[it] > 0) {
                        ...
                        checkArgument(dw > 1, format("dw must be > 1, was %s", dw));
                        checkArgument((leftExcludedW + rightExcludedW) <= 0.5, format("Excluded weight must be <= 0.5, was %s", leftExcludedW + rightExcludedW));
    ...
    public double getQuantile(double q)
    {
        ...
        checkArgument(weight[n - 1] > 1, format("Expected weight[n - 1] > 1, but was %s", weight[n - 1]));
        checkArgument(index <= totalWeight, format("Expected index <= totalWeight, but index:%s > totalWeight:%s", index, totalWeight));
        checkArgument(index >= totalWeight - weight[n - 1] / 2, format("Expected index >= totalWeight - weight[n - 1] / 2, but" +
    ...
    {
        SliceOutput sliceBuilder = new DynamicSliceOutput(toIntExact(estimatedSerializedSizeInBytes()));
